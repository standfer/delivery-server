<?php
include 'C:\xampp\htdocs\delivery\dto\Order.php';
...
function getProductsByOrder() {
    $orderRequest = json_decode($_POST['order']);

    $order = (isset($orderRequest->id)) ? $orderRequest : '';
    $id = $order->id;
    ...
    while ($rowOrder = mysql_fetch_array($queryResult)) {
        extract($rowOrder);
        $order = new Order($idOrder, $addressOrder, $latOrder, $lngOrder, $phoneNumber, 
                $cost, $isDelivered, $idWorkplace, $addressWorkplace, $latWorkPlace, 
                $lngWorkPlace, $priority, $odd, $orderNotes, $idClient, $clientName, $clientPhone
        );
